...
public class MainActivity extends Activity {
    ...
    private TextView statusView, logView, rxSummaryView;
    ...
    private final BluetoothGattCallback gattCallback = new BluetoothGattCallback() {
        @Override public void onConnectionStateChange(BluetoothGatt currentGatt, int status, int newState) {
            ...
            if (newState == BluetoothProfile.STATE_DISCONNECTED) {
                connected = false; clearGattQueue(); addEvent("STATE", "", "", "DISCONNECTED " + statusName(status));
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        appendLog("Orbis Watch Lab v0.6 iniciado | Android " + Build.VERSION.RELEASE + " API " + Build.VERSION.SDK_INT);
    }
    ...
    private void buildInterface() {
        ...
        TextView title = new TextView(this); title.setText("ORBIS WATCH LAB v0.6"); title.setTextSize(21); title.setTypeface(Typeface.DEFAULT_BOLD); root.addView(title);
        statusView = new TextView(this); statusView.setText("Inicializando Bluetooth..."); statusView.setTextSize(14); root.addView(statusView);
        LinearLayout row1 = new LinearLayout(this); row1.addView(button("PROCURAR", v -> startScan()), weight()); row1.addView(button("FORÇAR GATT", v -> forceGatt()), weight()); row1.addView(button("DESCONECTAR", v -> disconnectGatt()), weight()); root.addView(row1);
        ...
        LinearLayout markerRow = new LinearLayout(this); markerInput = new EditText(this); markerInput.setHint("Marcador: abri batimentos..."); markerInput.setSingleLine(true); markerRow.addView(markerInput, new LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 3)); markerRow.addView(button("MARCAR", v -> addMarker()), new LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1)); root.addView(markerRow);
        ...
        LinearLayout sendRow = new LinearLayout(this); hexInput = new EditText(this); hexInput.setHint("HEX conhecido"); hexInput.setSingleLine(true); hexInput.setInputType(InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_FLAG_CAP_CHARACTERS); sendButton = button("ENVIAR", v -> sendHex()); sendRow.addView(hexInput, new LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 3)); sendRow.addView(sendButton, new LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1)); root.addView(sendRow);
        logScroll = new ScrollView(this); logView = new TextView(this); logView.setText("LOG ORBIS\n"); logView.setTextSize(10); logView.setTypeface(Typeface.MONOSPACE); logView.setTextIsSelectable(true); logScroll.addView(logView); root.addView(logScroll, new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, 0, 1));
        setContentView(root); updateSendState();
    }

    private Button button(String text, View.OnClickListener listener) { Button b = new Button(this); b.setText(text); b.setTextSize(10); b.setOnClickListener(listener); return b; }
    ...
    private void startScan() {
        forceMode = false; manualDisconnect = false; connectionAttempt = 0; lastDevice = null;
    ...
    private void forceGatt() {
        forceMode = true; manualDisconnect = false; connectionAttempt = 0;
    ...
    private void runNextOperation() {
        ...
        operationRunning = true; handler.postDelayed(operationTimeout, OP_TIMEOUT_MS); boolean started = false;
        try {
    ...
    private void sendHex() {
        if (!connected || writeChannels.isEmpty()) { toast("Conecte o G28 primeiro"); return; }
        try {
    ...
    private void clearSession() { synchronized (rxBuffer) { rxBuffer.setLength(0); } synchronized (sessionEvents) { sessionEvents.clear(); } packetCounts.clear(); rxPacketCount = 0; sessionStartedAt = System.currentTimeMillis(); rxSummaryView.setText("RX: 0 pacotes | 0 padrões"); appendLog("SESSÃO DE CAPTURA LIMPA"); }

    private void shareLog() { String text; synchronized (rxBuffer) { text = "ORBIS v0.6 — RX\n\n" + rxBuffer + "\nLOG COMPLETO\n\n" + logBuffer; } shareText("Orbis v0.6 G28", text, "Compartilhar captura"); }
    private void shareJson() {
        try {
            JSONObject root = new JSONObject(); JSONArray events = new JSONArray(); JSONObject patterns = new JSONObject();
            root.put("app", "Orbis Watch Lab"); root.put("version", "0.6"); root.put("session_started_epoch_ms", sessionStartedAt); root.put("exported_epoch_ms", System.currentTimeMillis()); root.put("mtu", negotiatedMtu); root.put("rx_packet_count", rxPacketCount);
            ...
            shareText("Orbis v0.6 JSON", root.toString(2), "Compartilhar JSON");
    ...
    private void disconnectGatt() { manualDisconnect = true; forceMode = false; stopScan(); clearGattQueue(); if (gatt != null && hasConnectPermission()) { try { gatt.disconnect(); } catch (Exception ignored) { } try { gatt.close(); } catch (Exception ignored) { } } gatt = null; connected = false; writeChannels.clear(); refreshChannelSpinner(); setStatus("Desconectado"); appendLog("Conexão encerrada manualmente"); }
    private void closeGattOnly() { clearGattQueue(); if (gatt != null && hasConnectPermission()) { try { gatt.disconnect(); } catch (Exception ignored) { } try { gatt.close(); } catch (Exception ignored) { } } gatt = null; connected = false; updateSendState(); }
    private void updateSendState() { runOnUiThread(() -> { boolean e = connected && !writeChannels.isEmpty(); if (sendButton != null) sendButton.setEnabled(e); if (hexInput != null) hexInput.setEnabled(e); if (channelSpinner != null) channelSpinner.setEnabled(e); }); }
    ...
    private String propertiesText(int p) { StringBuilder b = new StringBuilder(); if ((p & 2) != 0) b.append("READ "); if ((p & 8) != 0) b.append("WRITE "); if ((p & 4) != 0) b.append("WRITE_NR "); if ((p & 16) != 0) b.append("NOTIFY "); if ((p & 32) != 0) b.append("INDICATE "); return b.toString().trim(); }
    private String statusName(int s) { if (s == 0) return "SUCCESS"; if (s == 133) return "GATT_ERROR_133"; if (s == 201) return "BUSY"; return "STATUS_" + s; }
